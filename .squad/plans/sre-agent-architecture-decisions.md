...
```mermaid
flowchart TB
    subgraph advisor["Agentic Ops Advisor"]
        direction TB
        core["Agent Core<br/>(GPT-4.1 via Azure OpenAI)"]
        subgraph tools["Tool Surfaces"]
            sql["sql_telemetry.py<br/>GPU, network, cost, incidents"]
            wc["work_context_stub.py<br/>Change events, decisions, runbooks"]
            act["action_stub.py<br/>Proposed changes, approval sim"]
            sre_tool["sre_agent.py ⬡ Phase 2<br/>REST chat tool<br/>ENABLE_SRE_AGENT=true"]
        end
        subgraph mcp_srv["MCP Server (Phase 1)"]
            mcp["work_context_mcp.py<br/>ENABLE_MCP=true"]
            aad_val["Azure AD Token<br/>Validation<br/>(audience + issuer + tenant)"]
        end
        core --> tools
        core --> mcp_srv
    end

    subgraph sre["Azure SRE Agent"]
        direction TB
        sre_core["SRE Agent Core<br/>(Container App + Managed Identity)"]
        sre_mcp["MCP Connector<br/>(documented, auto-recovery)"]
        sre_rest["REST Chat API<br/>/api/v2/chat<br/>(undocumented)"]
        sre_kb["Knowledge Base<br/>(portal upload, .md files)"]
        sre_core --- sre_mcp
        sre_core --- sre_rest
        sre_core --- sre_kb
    end
    ...
    subgraph auth["Auth Layer"]
        dac["DefaultAzureCredential<br/>(consistent with all tools)"]
        rbac["RBAC: SRE Agent<br/>Standard User<br/>(chat-only scope)"]
        res_id["Resource ID<br/>59f0a04a-b322-4310-<br/>adc9-39ac41e9631e"]
        dac --> rbac
        dac --> res_id
    end

    %% Phase 1: SRE Agent pulls context from us (documented)
    sre_mcp -- "Phase 1 · Pull Model<br/>SRE Agent → Us<br/>(documented, supported)" --> mcp
    aad_val -. "validates inbound<br/>MCP requests" .-> mcp

    %% Phase 2: We query SRE Agent (undocumented API)
    sre_tool -- "Phase 2 · Push/Query<br/>Us → SRE Agent<br/>(undocumented, synthetic fallback)" --> sre_rest
    dac -. "token acquisition<br/>per-request" .-> sre_tool

    %% Phase 3 (future)
    sre_core -. "Phase 3 · Sub-Agent<br/>Registration (optional,<br/>deprioritized)" .-> advisor

    %% Internet boundary
    internet{{"Public HTTPS Endpoint<br/>(VNet-restrict later)"}}
    sre_mcp --> internet --> mcp

    style advisor fill:#1a1a2e,stroke:#e94560,color:#fff
    style sre fill:#16213e,stroke:#0f3460,color:#fff
    style auth fill:#0f3460,stroke:#533483,color:#fff
    style internet fill:#e94560,stroke:#e94560,color:#fff
    style sre_tool fill:#533483,stroke:#e94560,color:#fff
...
```mermaid
gantt
    title SRE Agent Integration Phases
    dateFormat X
    axisFormat %s
    section Phase 1 (Now)
        Extend work_context_mcp.py           :done, p1a, 0, 2
        Add Azure AD token validation        :done, p1b, 0, 2
        MCP connector for SRE Agent          :done, p1c, 1, 3
    section Phase 2 (Next)
        Build tools/sre_agent.py skeleton    :active, p2a, 3, 5
        Synthetic fallback pattern           :active, p2b, 3, 5
        ENABLE_SRE_AGENT flag + config       :active, p2c, 3, 4
    section Phase 3 (Later)
        Custom sub-agent registration        :p3a, 5, 7
        REST v2 extended agent API           :p3b, 5, 7
    section Spike
        Test #remember via REST (30 min)     :crit, spike, 2, 3
```
...
sequenceDiagram
    ...
    participant MCP as MCP Server<br/>(work_context_mcp.py)
    participant Advisor as Agentic Ops Advisor
    participant REST as SRE REST API<br/>(/api/v2/chat)

    Note over SRE,MCP: Phase 1 — Pull Model (Documented)
    SRE->>MCP: MCP tool call (get change context)
    MCP->>MCP: Validate Azure AD token
    MCP-->>SRE: Change events, decisions, runbooks

    Note over Advisor,REST: Phase 2 — Query Model (Undocumented)
    Advisor->>Advisor: DefaultAzureCredential.get_token<br/>(59f0a04a-.../.default)
    Advisor->>REST: POST /api/v2/chat<br/>(Bearer token)
    REST-->>Advisor: Azure-native diagnostics
    alt SRE Agent unavailable
        Advisor->>Advisor: Return synthetic SRE response<br/>(fallback mode)
    end
```
