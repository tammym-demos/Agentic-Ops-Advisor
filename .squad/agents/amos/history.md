...
### 2026-04-14: Clean Up agent.yaml — Remove Manual Tool Schemas

**Session:** Task from Tammy
**Status:** ✅ IMPLEMENTED
**Focus:** Remove redundant manual tool JSON schemas from agent.yaml

**What was removed:**
- Entire `tools:` section (lines 63-183, ~120 lines) containing manual JSON schemas for:
  - `query_telemetry` — SQL telemetry function schema
  - `get_work_context` — Work IQ context function schema
  - `propose_change` — Action stub function schema
  - `request_approval` — Approval stub function schema
- These schemas are now auto-generated by the Agent Framework from Python function type annotations

**What was kept:**
- Agent metadata (name, description, version)
- Model configuration (gpt-4.1, api_version)
- Protocol settings (responses, v1)
- instructions_file reference (agent/system_prompt.md)
- Container configuration (image, port, health, resources, environment)
